...
public class Database1Impl implements IDatabase1 {
  ...
  public Database1Impl(BaseDatabaseConnection conn, IDatabases databases) {
    this.conn = conn;
    this.accessories = new BaseAccessoryPersistenceImpl(conn, databases);
    this.events = new BaseEventPersistenceImpl(conn, databases);
    this.events_rooms = new BaseEventsRoomPersistenceImpl(conn, databases);
    this.groups = new BaseGroupPersistenceImpl(conn, databases);
    this.groups_people = new BaseGroupsPersonPersistenceImpl(conn, databases);
    this.people = new BasePersonPersistenceImpl(conn, databases);
    this.rooms = new BaseRoomPersistenceImpl(conn, databases);
    this.school_terms = new BaseSchoolTermPersistenceImpl(conn, databases);
    this.subjects = new BaseSubjectPersistenceImpl(conn, databases);
    this.term_settings = new BaseTermSettingPersistenceImpl(conn, databases);
    this.user_settings = new BaseUserSettingPersistenceImpl(conn, databases);
  }
